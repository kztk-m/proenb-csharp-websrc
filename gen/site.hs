...
tocTemplate =
  either error id . runIdentity . P.compileTemplate "" $
    T.unlines
      [ "<nav class=\"toc\">"
      , "$toc$"
      , "</nav>"
...
pageCompilingRule = do
  ...
  case checkBN bn of
    Just (Q i) -> do
      inst <-
        if i <= 4
          then load "questions/instruction.md"
      ...
      let ctxt' =
            constField "instruction" (itemBody inst)
              ...
              <> constField "lastModified" (formatTimeAsJST $ max mt mti)
              <> ctxt -- update lastModified
    ...
    Nothing -> do
      let tmpl =
            if "setup" `Data.List.isPrefixOf` bn
              then

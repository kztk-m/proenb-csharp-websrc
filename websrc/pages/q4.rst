...
**./items.txtも提出物に含めること．** ただし，items.txt は .csproj ファイルと同じディレクトリに置くものとし，その状態でプログラムから相対パス``./items.txt``で当該ファイルにアクセスできるようにするために，以下のいずれかを実施せよ．
   ...
   - Windows版のVisual Studioを使用している場合は，プロジェクト名を右クリックして出てくるコンテキストメニューから「プロパティ」を
     選択し，「デバッグ」の「作業ディレクトリ」を当該 .csproj ファイルを直上のディレクトリに設定する．

   - Mac版のVisual Studioを使用している場合は，プロジェクト名をダブルクリックして出てくるウィンドウの「実行」>「構成」> 「Default」の 
     「Working directory」（作業ディレクトリ）を当該 .csproj ファイルを直上のディレクトリに設定する．

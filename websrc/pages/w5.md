...
本演習では後者の部分は変更せずに，前者の`MainForm.cs`を拡張することで進めていく．
...
public MainForm() 
{
    ...
    // 配置先のStackLayout自体の伸縮に合わせて縦に伸縮するTextArea．
    // 伸縮しないようにするには二つ目の引数をfalseにする．
...
![ウィンドウ．ラベル等がタイル状に配置されている．中央の行および列がウィンドウのサイズの伸縮に合わせて伸縮する．](images/eto/table_layout.png)\

...
複数個のデータをひとまとめ（コレクション）にして取り扱いたいことはよくある．
...
> 本演習ではこの区別には深入りしないが，興味のある人は <https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/> や，
> そこから辿れる文章等を調べてみるとよい．
...
class ListExample
{
    static void Main()
    {
        ...
        // 指定された要素を取り除くことができる．
        // 指定された要素が複数出現する場合は最初のものを取り除く．
        // ただし，O(max(要素のインデックス,Count - 要素のインデックス))時間かかる
        ns.Remove("mikan");
...
| ``bool Remove(T)``     | 与えられた要素と同じ要素のうちもっとも最初に出現するものを取り除く．返り値は削除に成功した場合は``true``，そうでなければ（典型的には与えられた要素がリストに存在しない場合）``false``． |
...
| 名前                          | 説明                                                                   |
| :---------------------------- | :--------------------------------------------------------------------- |
| ``PlayList(string[] titles)`` | 曲目が``titles``で与えられるようなプレイリストを作成するコンストラクタ． |
...
より正確な言い方をすれば，``foreach``を使用するには``in``の右に書くオブジェクトが，``System.Collections.Generic.IEnumerable<T>`` インタフェース （か，その非ジェネリック版の``System.Collections.IEnumerable``）を実装している型である必要がある．
...
**インタフェース** は，大雑把にはオブジェクトが持っているべきインスタンスメソッドやプロパティ等を規定するものである．
クラスが，これらのインタフェースの規定するメソッドやプロパティを実装しているとき，そのクラスが当該インタフェースを **実装している** と言う．
...
インタフェースも継承と同じく再利用性を高める．たとえば``foreach``文はよい例であり，``IEnumerable<T>``（か，その非ジェネリック版の``IEnumerable``）を実装している型ならばどんな型に対しても``foreach``文を利用可能である．
...
そうでなければ，仮想メソッドである``Object.Equals`` メソッドが用いられる（[参考](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.remove?view=net-10.0#system-collections-generic-list-1-remove(-0))）．
...
```cs
// コレクション初期化子を伴う場合は，引数なしコンストラクタの()は省略可

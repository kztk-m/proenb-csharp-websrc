...
表している．また， *private修飾子* はこのフィールドがこのクラスの外からはアクセスできないことを表している．``public Counter(int c0) {…}``の部分はこのクラスの **コンストラクタ** を定義している．コンストラクタはオブジェクトを生成する際に呼ばれ，ここではフィールド``count``を``c0``で初期化している．
...
（インスタンス）メソッド``Method``はオブジェクト``o``に対し，``o.Method(arg1, arg2)``のような形で呼出すことができる関数のようなものである．
...
メソッド``Inc()``等が，``obj.Inc()``の形で呼び出されていることを確認しよう．また，（インスタンス）メソッド``Inc()``の定義において（インスタンス）フィールド``count``が参照されていることにも注意する．大雑把には，この``count``は，メソッド``Inc()``が属するインスタンスの同名のフィールドを指している．
...
一方で，このような処理を``Counter``クラスの（インスタンス）メソッドとして実装することはできない．これは， *インスタンスメソッドはインスタンスに属する*
...
さて，この関数の実装であるが，
``Int32.Parse``メソッド（参考：[.NETのAPIリファレンス](https://learn.microsoft.com/en-us/dotnet/api/?view=net-10.0)）を用いることで実装できる．
...
> [!IMPORTANT]
>
> 本演習を実施の際はCopilot等のAI支援機能をオフにしてください．

> [!CAUTION]
>
> 練習問題の解答は提出しない．講義中にトライしわからない点を質問するのに使うとよい．

カウンターの例を実際に手元で動かして動作を確認してみよう．また，以下の`public`メソッドを追加してみよう．

| メソッド | 説明 |
| :-- | :-- |
| `void SetCount(int n)` | `count`を`n`に設定する |
| `void IncMany(int n)` | `n`回`Inc()`する |

以降に記載の情報も参考にするとよい．どのように動作を確認するかも考えてみよう．

...
``List<T>``や配列など，``Add``メソッドを持ち，なおかつ``IEnumerable``を実装しているクラス（``IEnumerable<T>``でもよい）に対しては，
**コレクション初期化子** を利用することができる．配列に対するコレクション初期化子は実は既に登場していて

.. code:: cs

   int[] nums2 = new int[] { 1, 2, 3, 4, 5 };

では配列に対するコレクション初期化子が使用されている．同様に

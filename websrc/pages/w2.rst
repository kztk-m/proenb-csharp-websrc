...
`前回 <./w1.html>`_，オブジェクトというのは大雑把には「データと，それに関連する関数」をまとめたものであり，
...
アクセス指定子を適切に設定することにより，関数やデータをプログラムの他のパートから隠蔽することができる．
...
.. note:: 
   ...
   一方で，クラスではうまくまとめきれない感心事をまとめるためにアスペクト指向言語などの言語が提案されている．
...
..
   ...
   という単純なコードについては``a``が``int``型の変数であっても``double``型の変数であっても``string``型の変数であっても動作する．この挙動には
   **オーバロード** （具体的には+がstringとobject型の引数を受けとったときにstringを返すように振る舞うこと）および
...
継承は，元となるクラス``B``（基底クラス，親クラスなど）からアクセスレベルがprivateでない（protected，internalや publicなど）なフィールドやメソッド等を
すべて受け継いだクラス``D``（派生クラス，子クラスなど）を作成する手法である．
その際において以下が可能となる．

  - ``D``のインスタンスは``B``のインスタンスして扱うことができる（``D``は``B``の **部分型（subtype）** ）
  - クラス``B``の **仮想メソッド（virtual method）** を クラス``D``は **オーバライド（override）** することができる．
    * ``D``が``B``の仮想メソッド``f()``をオーバライドしているしている場合，以下で呼び出されるメソッドは
...
.. code:: cs 
   ...
   class MobilePhone : Phone
   {
       ...
       // ": base(n)" の部分は基底クラスのコンストラクタを呼ぶ．ここでは Phone(n) 
       // 省略は ": base()" と同じ意味である．
       // そのため，もしここで省略すると Phone()は定義されていないためエラーになる．
...
前述したように，MobilePhoneのインスタンスはPhoneのインスタンスでもある．
...
というコードは``p``に代入されるものがPhoneのインスタンスであろうがMobilePhoneのインスタンスであろうが動作する．
このように同じコードで異なるデータを扱えることをポリモーフィズムと言う．

もう一つは``Info()``は仮想メソッドであるので``p.Info()``の挙動は実際に``p``に代入されるインスタンスにより変わりうる．
現時点では``Phone``の派生クラスは``MobilePhone``しかないが，別の派生クラスを定義したり``MobilePhone``をさらに派生したりすること
も可能である．
...
以上のコードが``a``と``b``と``c``の型が違うにもかかわらず動作するのは，``Console.WriteLine(object)``が中で``ToString()``を呼んでいるためである．
...
たとえば，`前回 <./w1.html>`_の``Counter``クラスに以下のコンストラクタやメソッドを追加することができる．
...
.. note::
   ...
   継承によるポリモーフィズムは部分型ポリモーフィズム（subtype polyrmorphism）と呼ばれるものの一種であるが，
...
定数がキャストされるときにキャスト先の型の収まらない場合は **例外** （後の回で詳しく紹介する．捕捉しなければエラーとなる）が発生する．
...
.. admonition:: 例
   ...
   .. code::
      ...
      Console.WriteLine((sbyte) bb); // -1

また，クラスについてはキャスト元がキャスト先の型を持ちえない場合はやはり例外が発生する．
...
上述の``MobilePhone``クラスをさらに継承し，以下の仕様を満たすクラス``SmartPhone``を作成しなさい．

...
この`Calc`の定義は`Program.cs`ではなく`Calc.cs`というファイルに含めることにする．この`Calc.cs`は`Program.cs`と同じフォルダに置くものとする．**提出するのは`Program.cs`のみであり`Calc.cs`は提出物に含めない**．
...
計算等はせずに `Error!` と表示するようにせよ． 具体的には以下のようにメソッドの挙動を変更せよ．ただし，`Calc`クラスを **変更してはならない．**

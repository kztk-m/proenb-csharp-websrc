...
> [!NOTE]
>
> 余裕があれば次も試してみよう：`f(Phone)`と`f(MobilePhone)`とオーバロードされた関数があるときに，
> `MobilePhone`のインスタンスを渡すとどちらの関数が呼ばれるか？`Phone`にキャストするとどうなるか．

> [!NOTE]
>
> さらに余裕のある人は`override`を付けなかったときにどのような挙動になるか確認してみよう．

...
``this[TKey]``                   インデクサ．``dict[k] = v``は``k``が存在しない場合は``dict.Add(k,v)``と同様．存在する場合はバリューを``v``で上書き．
...
``File.ReadAllLines(string,string[])``    static メソッド．与えられたパスのファイルを開いて，与えられた文字列の配列で上書きして閉じる．パスにファイルが存在してなければ作成する．

...
``this[TKey]``                   インデクサー．``dict[k] = v``は``k``が辞書に存在しない場合は``dict.Add(k,v)``と同様．存在する場合はバリューを``v``で上書き．
                                 ``dict[k]``を式として使うときはキーが辞書に存在してなければ例外発生．
...
.. important::
   ...
   プログラム中の相対パス（``./MyText.txt``等）の基準は **作業フォルダ（作業ディレクトリ）** である．作業フォルダは `環境設定1 <./setup.html>`_ で紹介した方法でプロジェクトを作成し・プログラムを実行しているのならば，VS Codeで開いたフォルダ（プロジェクトルート，つまり.csprojファイルがあるフォルダ）が作業フォルダになるはずだ．なので，相対パス``./MyText.txt``はVS Codeで開いたフォルダ直下に置かれた``MyText.txt``を指す．

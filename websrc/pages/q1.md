...
class EmoneyCard 
{
    ...
    public bool Pay(int b) 
    {
        ...
        // https://learn.microsoft.com/ja-jp/dotnet/csharp/misc/cs0161
        return false; 
    }
    public void Print() 
    {
        ...
        // たとえばカードの名前（cardName）が "MyCard" で残高（balance）が 500 であれば，
...
| メソッド                                         | 実装すべき挙動                                                                                                                                                  |
| :----------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `public bool TransferFrom(EmoneyCard ec, int b)` | `ec1.TransferFrom(ec2,b)`は，`ec2`の残高が`b`（≧ 0と仮定してよい）以上であれば，`ec2`の残高を`b`減らし，`ec1`の残高を`b`増やし，`true`を返す．そうでなければ，`false`を返す． |

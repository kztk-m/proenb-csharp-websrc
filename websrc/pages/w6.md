...
![DrawableDemoの実行例．直線，矩形，楕円などが描画されている](images/eto/drawable_example.png)\
...
| `SizeF(float, float)` | `Size`の`float`版，`SizeF`のコンストラクタ．高さと幅にはプロパティ`Height`と`Width`からアクセス可． |
...
なお，上記はあくまで一部である．より詳細は[Eto.FormsのドキュメンテーションのGraphicsのページ](http://pages.picoe.ca/docs/api/html/T_Eto_Drawing_Graphics.htm)を見るとよい．
...
List<FontFamily> fontfamilies = new List<FontFamily>( Fonts.AvailableFontFamilies ); 
```
...
d.Paint += (s, pe) =>
{
    ...
    if (ff != null)
    {
        // sizeSelector.Value は double なので int に丸める．
    ...
    public class MainForm : Form
    {
        public MainForm()
        {
            ...
            d.Paint += (s, pe) =>
            {
                ...
                if (ff != null)
                {
                    pe.Graphics.DrawText(new Font(ff, (int)sizeSelector.Value), 
                                         Colors.Black, new PointF(0, 0), textToDraw.Text);
                }
            };
...
![FontDemoの実行例．ドロップダウンメニューで選択したフォントファミリ，ステッパーでフォントサイズで，テキストボックスに入力した文字列が，ウィンドウ下部のDrawableに描画されている](./images/eto/font_demo.png)\
...
| プロパティ | 説明 |
| :-- | :-- |
| `Buttons` | マウスのボタン（`MouseButtons.Primary`や`MouseButtons.Alternate`など．同時押しもOK） |
...
MouseUp += (s, me) => {
   ...
   else if ( me.Buttons.HasFlag(MouseButtons.Alternate) )
   {
       // 右クリックのときの処理
   }
}
...
namespace MouseDemo
{
    public class MainForm : Form
    {
        public MainForm()
        {
            ...
            // クリックされた位置．初期位置は描画されないように (-100, -100)．
            // null許容値型（nullきょよう あたいがた）を使って
            // PointF? clicked = null;
            // としたほうが意図が明確だが，nullチェックが必要になって面倒でもある．
            PointF clicked = new PointF(-100, -100); 
            ...
            d.Paint += (s, pe) =>
            {
                // 演算子のオーバローディングにより，floatをSizeFに掛けることもできるし，
                // PointFにSizeFを加減算可能．
                pe.Graphics.FillEllipse(Colors.Black, 
                                        new RectangleF(clicked - 0.5F * circleSize, circleSize));
            };
            d.MouseDown += (s, me) =>
            {
                clicked = me.Location;
...
![MouseDemoの実行例．黒く塗られた小さな円が表示されている](./images/eto/mouse_demo.png)\
...
より具体的には，`ColorPicker`により *次にクリックしたとき* に表示される円の色を指定できるようにする．
...
class ColoredCircle
{
    ...
    public void Draw(Graphics g)
    {
        ...
        g.FillEllipse(Color, new RectangleF(Center - 0.5F * circleSize, circleSize));
    }
}
```
...
namespace MouseDemo2 
{
    class ColoredCircle
    {
        ...
        public void Draw(Graphics g)
        {
            SizeF circleSize = new SizeF(10, 10);
            g.FillEllipse(Color, 
                          new RectangleF(Center - 0.5F * circleSize, circleSize));
        }
    }
...
上のコードのMacでの実行例はたとえば以下となる．

![MouseDemo2の実行例．橙色で塗られた小さな円が表示されている．次に描画される円の色を表すカラーピッカーではマゼンタが選択されている](./images/eto/mouse_demo2.png)\

## 複数個の円を描く

さて，円が一個だけでは寂しいのでもっとたくさん描画できるようにしたい．そこで，クリックするたびに円を描くようにしたい．この実現方法は2通りある．
...
![MouseDemo3の実行例．様々な色の円が複数表示されている](./images/eto/mouse_demo3.png)\
...
namespace MouseDemo4
{
    class DrawableImage : Drawable
    {
        ...
        {
            // ClientSizeのビットマップを作成．透明度付き
            // PixelFormat.Format32bppRgba は1画素が32ビット（4バイト）で表されていて，
            // 上位から順に アルファ，赤，緑，青 がそれぞれ8ビット（1バイト）ずつ並んでいることを表す．
            bitmap = new Bitmap(Math.Max(ClientSize.Width, 1), Math.Max(ClientSize.Height, 1),
            ...
            MouseDown += (s, me) =>
            {
                ...
                {
                    g.FillEllipse(CurrentColor, new RectangleF(center - 0.5F * circleSize, circleSize));
                }
                Invalidate();
            };
            SizeChanged += (s, e) =>
            {
                ...
                if (bitmap.Height < ClientSize.Height || bitmap.Width < ClientSize.Width)
                {
                    ...
                    // 毎回毎回リサイズで描画領域が増えるごとにコピーするのはコストが大きいので，
                    // 50%分の余裕をもたせる．
...
実行例は上と変わらないので割愛．ただし，Retina等の論理ピクセルと物理ピクセルが一致しない環境では，こちらのやり方だと円の表示がリストを使うやり方に比べ荒くなる．（防ぐには`Form`のプロパティ`LogicalPixelSize`を取得し，`LogicalPixelSize`倍大きな画像を用意し，`LogicalPixelSize`分の1に縮小して表示するようにする．）

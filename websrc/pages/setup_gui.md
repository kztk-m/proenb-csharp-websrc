...
具体的には，「[コマンドライン環境の設定](#cl_setup)」を行った後，VSCode上で[動作確認](#check)を行う．
...
1. 以下のコマンドを実行する（設定自体はこれで終わり）．8以前の.NET SDKを使っている場合は`@`の代わりに`::`と書く必要があるかもしれない．
   ...
   > [!NOTE]
   > 2.8.0〜2.8.2には [テンプレートがコンパイルできない](https://github.com/picoe/Eto/issues/2619) という不具合があるので，
...
1. （Macの場合のみ）これまでにインストール済でなければ，以下のコマンドを実行してXcode command line toolsをインストールする．
   ...
   いままでにXcodeをインストールしていなければ数GBほどのダウンロードが発生する．
...
1. 確認のために以下を実行してみる．
   ...
   > [!NOTE]
   > ``dotnet``のバージョンによっては，``dotnet new -l eto``や``dotnet new eto -l``のように書かなければならないかもしれない．
...
1. Eto.Formsプロジェクトを作成し，VSCodeで開く．
   ...
   i. 適当なフォルダ（``HelloEto``とする）を作成する．ただし，*プロジェクトフォルダ*の中を避ける．そして，VSCodeでそのフォルダを開く．

      > [!CAUTION]
      > プロジェクトフォルダはネストさせない．

   i. VSCode上のターミナル（メニューバーより「表示」→「ターミナル」あるいはJapanese Language Packを入れていないなら"View"→"Terminal"）
      を開き，以下のコマンドを実行する．``-sln``は不要だが，どうせC# Dev Kitが生成することになるので先に生成しても同じ．

      ```console
      dotnet new etoapp -sln 
      ```

      その後``ls -F``（Windowsだと``dir``）してみると以下のようなフォルダおよびファイルが生成されたことがわかる（Windowsだと表示は異なるが作成されるフォルダ・ファイルは同じ）．
...
1. Eto.Forms 2.11.0の不具合のワークアラウンドを実施する．**この作業はプロジェクト作成ごとに必要**になる．新しいバージョンでは解消されているかもしれない．変更する場所は`HelloEto`以下は以下の構造になっているので，Windowsの場合は`HelloEto.Wpf.csproj`，Macの場合は`HelloEto.Mac.csproj`である．
   ...
   * `HelloEto`
     * `HelloEto`
       * ...
     * `HelloEto.Wpf`
       * `HelloEto.Wpf.csproj`（**← Windowsの場合はこれを変更**）
       * ...
     * `HelloEto.Mac`
       * `HelloEto.Mac.csproj`（**← Macの場合はこれを変更**）
       * ...
     * ...
   :::

   * Windowsの場合，`HelloEto.Wpf`直下の`HelloEto.Wpf.csproj`を開き，`PropertyGroup`内の以下の箇所

     ```xml
     <TargetPlatform>net10.0</TargetPlatform>
     ```

     を下記のものに変更する（`net10.0`の部分はインストールしたSDKのバージョンに合わせる）．要は，`-windows`を追加する．

     ```xml
     <TargetPlatform>net10.0-windows</TargetPlatform>
     ```

   * また，Macの場合は，`HelloEto.Mac`直下にある`HelloEto.Mac.csproj`を開き，`PropertyGroup`内に以下の行を追加する．

     ```xml
     <StartWorkingDirectory>./</StartWorkingDirectory>
     ```

     ターミナルからのみ実行する場合はこの作業は不要．
...
1. ビルドおよび実行してみる．
   ...
   はたまた，「実行とデバッグ」あるいは``F5``キーで（デバッグ）実行する場合は，やはり「デバッガーの選択」や「起動構成の選択」等について聞かれる．最初の質問には「C#」と回答すればよいが，
   二つ目の質問にはWindowsの場合は「HelloEto.Wpf」，Macの場合は「HelloEto.Mac」と回答する． [GTKのインストール](#setup_gtk) を行っていれば「HelloEto.Gtk」を選んでもよい．
   ...
   * Macの場合の挙動
     ...
     i. ツールバー上のボタン「Click Me!」は初期ウィンドウサイズだと隠れてしまっている．拡大して表示しよう．
     ...
     i. デバッグ実行だと，Command + Qを押したりメニューからQuitを選択したりしてもデバッグ実行自体は終了しない．
...
1. [Homebrew](https://brew.sh/)をトップページに書いてあることに従いインストールする．遅かれ早かれ``brew``は入れることになると思うので，最終的な手間はそう大きくないはず．

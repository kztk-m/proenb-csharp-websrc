...
そのため，あなたのタスクは前者の，ファイルを読んでその内容から`Dictionary<string, int>`を作成することである．この処理を実現するため，以下のプログラムの空欄を埋めて完成させよ．
...
class Program
{
    static Dictionary<string, int> readDictionary(IEnumerable<string> lines) 
    {
        ...
        foreach(var line in lines) {
            // ...
            // この中でlinesを変更しない．そもそも参照する必要はないはず．
        }
    ...
    static void procFile(string filepath)
    {
        ...
        // linesから辞書を作成し，その辞書をバリューが降順になるようなキーの順で標準出力に出力する
        // 前者をするのが readDictionary．この実装の完成が課題．
        // 後者をするのが DictionaryHelper.PrintByDescendingValues．こちらは所与．
...
1. まず，入力ファイルを準備する（`q4_items1.txt`という名前だとする）．VSCodeで以下の中身の`q4_items1.txt`を`.csproj`と同じフォルダに作成する．
...
> [!IMPORTANT]
>
> `dotnet run`を実行する前に，編集中のプログラムファイルに未保存の変更がないかを確認し，あれば（そしてそれが意図通りでなければ）保存する．そうでなければ，書いているものと実行しているものが違うことになる．

:::

**自身がプログラムの動作確認に使用した入力ファイル（上記の`q4_items1.txt`）も一つ以上提出物に含めること（発展課題も同じ）**．動作利便性および環境由来のトラブル回避のため，ファイル名の拡張子を除いた部分に使用してよい文字は英語アルファベット小文字，`_`，数字のみとする．
...
* 品目名は`,`を含まず，さらに先頭も末尾も空白でない．また改行も含まない．たとえば<code> </code>（空白文字からなる文字列）や<code> X </code>（空白文字，'X'，空白文字からなる文字列）などはここで言う品目名ではない．先頭末尾以外の（非改行）空白は含んでもよい（例：`Ruby Chocolate`，`Café au lait`）．なお，*空文字列も有効な品目名である*ことに注意（`,`を含まず，先頭も末尾も空白でないので）．大文字小文字は区別する．
...
「ファイルが正常に開けなかった旨」は最低「ファイルが正常に開けませんでした」と表示したのでもよい．例外あるいは例外の型そのままを出力するのは避ける．それはこれらがC#の実装の詳細であるためである．ユーザの立場での原因が分かるのならばその情報を表示したほうが親切である．
...
<figure>
<figcaption>入力ファイル：[q4_items2.txt](./q4_items2.txt)</figcaption>
```text
Chocolate,2
Chips,14
Candy,5
...
<figure>
<figcaption>入力ファイル：[q4_items3.txt](./q4_items3.txt)</figcaption>
...
<figure>
<figcaption>入力ファイル：[q4_items4.txt](./q4_items4.txt)</figcaption>
...
<figure>
<figcaption>入力ファイル：[q4_items5.txt](./q4_items5.txt)</figcaption>

```text
# 品目名は先頭末尾以外の空白を含んでいてよい
Ruby Chocolate, 6
# 品目名が数字なのは許される
100 , 5
# 品目名に#は許される
...
<figure>
<figcaption>入力ファイル：[q4_items6.txt](./q4_items6.txt)</figcaption>

...
* 個数は`int`で表現可能な数に比べて十分に小さいとする（オーバフローを考える必要はない）．
...
> [!NOTE]
> ``items.txt``を相対パス``./items.txt``でアクセスするためには，``items.txt``は作業ディレクトリ直下に置く必要がある．[環境設定1](./setup.html)で紹介
> した方法でプログラムを実行しているのならば，VS Codeで開いたフォルダ（プロジェクトルート）直下に置いたので問題がないはずだ．
>
